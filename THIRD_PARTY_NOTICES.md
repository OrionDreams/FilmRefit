...
FilmRefit includes, bundles, or is built using third-party open-source software.

This file summarizes the principal third-party components distributed with official
FilmRefit builds. Each third-party component remains subject to its own license.
Nothing in this file changes the license of FilmRefit itself or of any third-party
software.
...
FilmRefit bundles `ffmpeg` and `ffprobe` as standalone executables for probing and
...
FilmRefit intentionally uses redistributable GPL builds with `--enable-gpl` and
`--enable-version3`.
...
Official FilmRefit Apple Silicon releases use the pinned build published by:
...
Official FilmRefit Intel macOS releases use the FFmpeg 9.0.1 static binaries
...
enabled, as in the FilmRefit builds described above, the resulting FFmpeg binaries
...
FilmRefit release packages SHOULD preserve the exact FFmpeg build information used
...
FilmRefit uses the Avalonia UI framework.
...
FilmRefit receives the Inter font through `Avalonia.Fonts.Inter`.
...
FilmRefit is published as a self-contained .NET 10 application. Official packages
...
For release compliance, FilmRefit SHOULD ship the exact .NET runtime license and
...
FilmRefit's standalone transcoder executable is built from CPython using PyInstaller.
...
FilmRefit release packaging SHOULD preserve the CPython license corresponding to
the exact Python version used to build the transcoder.

---

## PyInstaller

FilmRefit uses PyInstaller to create the standalone `filmrefit-transcoder` executable.
...
FilmRefit's project configuration excludes this package from ordinary non-Debug
...
part of normal FilmRefit release packages.
...
Official FilmRefit packages should preferably contain a layout similar to:
...
Before publishing an official FilmRefit release:

1. Include FilmRefit's own `LICENSE`.
...
7. Preserve Python and PyInstaller license files corresponding to the exact
   transcoder build.
