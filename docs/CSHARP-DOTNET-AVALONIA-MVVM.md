...
- **Output validation:** A zero exit code from the transcoder is not enough to mark a proxy or mezzanine as successful. After each successful subprocess exit, probe the generated output through `transcoder/filmrefit.py --probe-json` and compare video frame counts exactly when both source and output expose stream `nb_frames`. If frame counts are unavailable, compare duration exactly as a fallback. If the output is missing, cannot be probed, has unknown duration, or does not match, record a failed Status row and keep the batch progress bar in its error state.
