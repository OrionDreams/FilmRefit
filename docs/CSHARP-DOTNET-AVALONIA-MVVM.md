...
- **Output validation:** A zero exit code from the transcoder is not enough to mark a proxy or mezzanine as successful. After each successful subprocess exit, probe the generated output through `transcoder/filmrefit.py --probe-json` and compare its duration with the original clip duration. Allow up to one second of difference to absorb container/frame-rate rounding. If the output is missing, cannot be probed, has unknown duration, or differs by more than the tolerance, record a failed Status row and keep the batch progress bar in its error state.
...
- **Target artifacts:** Early release targets are Windows x64 `.zip`, Linux x64 `.AppImage` plus `.tar.gz` fallback, and macOS `.dmg` for Apple Silicon / Intel. This is open to changing depending on how dependencies such as ffmpeg can be delivered.
